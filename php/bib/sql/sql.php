<?php 
	...
	function getBatchJobs() {
		...
		$sql = "SELECT ID, Name, Von, Pfad, CONVERT(nchar, createdAT ) as date FROM [SQLJob]";
		$DB->set_sql($sql);	
		$DB->_query();		
		...
		foreach( $result as $value ){						
			...
			echo '<a href="./sql.php?id='.$value['ID'].'">';
			echo '<p>'. $value['Name'] .'</p>';
			echo '</a>';
			echo '</div>';
			echo '<div id="info">';
	...
	function getBatchJobInfos() {
		...
		$DB->set_database("Lindorff_DB");
		$DB->_connect();		
		$sql = "SELECT Name, Von, CONVERT(nchar, createdAT ) as date, ID, Beschreibung, Pfad
				FROM [SQLJob]
				WHERE ID = ".$_GET['id']." ";
		$DB->set_sql($sql);	
		$DB->_query();		
...
<link rel="stylesheet" type="text/css" href="../../../css/bib/sql/sql.css">
...
$(document).ready(function() {
	...
	$("#create button[name='Speichern']").click( function(event) {
		...
		$.each( Felder, function() {
			if( $(this).val() == '' ) {
				$(this).attr("placeholder","Bitte einen Wert angeben.").css("color", "red");
				counter += 1;
			} else {
				counter - 1;
		...
		if(answer) {						
			$.ajax({
				...
				success	: function(data) {										
					...
					setTimeout(function(){ window.location.href = "http://localhost/Tool/php/bib/sql/sql.php" }, 3000);
				}
			});
		}
	});
...
echo '
	<div id="content_header">
		<img src="../../../img/icons/sql_file.png" name="img_header">
		<h1>SQL - Skripte</h1>
		<a class="folder" href="file:///C:\xampp_1.8.2\htdocs\Tool\Skripte\sql"></a>
		<p class="info">Hier steht eine Auswahl von angelegten Skripten zur Verfügung.</p><br />	
		<p class="info">Um ein SQL - Skript zur Datenbank hinzuzufügen, einfach auf das <b>+</b> klicken.</p>
		<a href="./sql.php?option=create"><i class="icon-plus"></i></a>
	</div>
...
';
	...
	if( isset($_GET['id']) ) {
		...
		echo '
		<div id="create">
		<form enctype="multipart/form-data" action="ajaxSql.php" method="POST" >
			<h1>Skript hinzufügen: </h1>
			<p class="create_info">Hier können Skripte hinzugefügt werden. <br />Skripte werden automatisch in den Ordner /Skripte/sql verschoben.</p>
			<div id="Name">
				<h2>Name: </h2>
